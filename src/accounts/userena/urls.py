...
urlpatterns = [
    ...
    url(r'^password/reset/$',
        ...
        merged_dict({'template_name': 'password_reset.html',
    ...
    url(r'^password/reset/confirm/(?P<%s>[0-9A-Za-z]+)-(?P<token>.+)/$' % password_reset_uid_kwarg,
        ...
        merged_dict({'template_name': 'password_reset_confirm.html',
    ...
    url(r'^id-(?P<username>[\@\.\w-]+)/email/$',
        userena_views.email_change,
        {'template_name': 'email_change.html'},
        name='userena_email_change'),
    url(r'^id-(?P<username>[\@\.\w-]+)/email/complete/$',
        userena_views.direct_to_user_template,
        {'template_name': 'email_change_done.html'},
        name='userena_email_change_complete'),
    url(r'^id-(?P<username>[\@\.\w-]+)/confirm-email/complete/$',
        userena_views.direct_to_user_template,
        {'template_name': 'email_confirm_done.html'},
    ...
    # Disabled account
    url(r'^id-(?P<username>[\@\.\w-]+)/disabled/$',
        userena_views.disabled_account,
        {'template_name': 'userena/disabled.html'},
        name='userena_disabled'),

    # Change password
    url(r'^id-(?P<username>[\@\.\w-]+)/password/$',
        userena_views.password_change,
        {'template_name': 'password_change.html'},
        name='userena_password_change'),
    url(r'^id-(?P<username>[\@\.\w-]+)/password/complete/$',
        userena_views.direct_to_user_template,
        {'template_name': 'password_change_done.html'},
        name='userena_password_change_complete'),

    # Edit profile
    url(r'^id-(?P<username>[\@\.\w-]+)/edit/$',
        userena_views.profile_edit,
        {'template_name': 'profile_edit.html'},
        name='userena_profile_edit'),

    # View profiles
    url(r'^id-(?P<username>(?!(signout|signup|signin)/)[\@\.\w-]+)/$',
